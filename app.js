...
console.log('Exercise 1:', pokemon[58]);
...
*/
const starterPokemon = pokemon.find(pokemon => pokemon.starter === true); // filter through but do not know how to choose any but the first

game.party.push(starterPokemon);

// console.log(starterPokemon);

console.log(`Excersie 4:` , game.party);
...
starterPokemons.forEach(pokemon => {
    console.log('Excercise 9' , pokemon.name);
});
/*
Exercise 10
...
catchPokemon(pokemon[53]);

console.log(game.items);  //Went from 8 pokeballs to 7
...
*/
game.gyms.forEach(gym => {
    if(gym.difficulty >= 3 && gym.difficulty < 6) {           //same code just added x=difficulty... 3<=x<6
        gym.completed = true
    }
})
...
game.gymStatus = () => {
    ...
    game.gyms.forEach(gym => {
        if (gym.completed) {
            ...
            gymTally.incomplete += 1;             // need more practice with foreach. took way to long. used ++ at first but that was counting the word incomplete.....so made the correction to cheack true or false status
        }
    });
    console.log(gymTally);                      //should work now
};
game.gymStatus();
...
console.log('Exercise 14:' , game.partyCount() , 'Pokemon currently in Party');
...
*/
game.gyms.forEach(gym => {
    if(gym.difficulty >= 6 && gym.difficulty <= 8) {           //same code just added x=difficulty... 6<=x<=8
        gym.completed = true
    }
})

console.log(`Excercise 15` , game.gyms);
